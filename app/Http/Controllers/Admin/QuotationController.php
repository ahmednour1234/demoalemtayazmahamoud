...
public function create(Request $request)
{
    ...
    if (!in_array("quotation.create", $decodedData)) {
        Toastr::warning('غير مسموح لك! كلم المدير.');
        return redirect()->back();
    }   
...
public function store(Request $request)
{
    ...
    if (!in_array("quotation.store", $decodedData)) {
        Toastr::warning('غير مسموح لك! كلم المدير.');
        return redirect()->back();
    }   
        $request->validate([
    ...
    if (!in_array("quotation.index", $decodedData)) {
        Toastr::warning('غير مسموح لك! كلم المدير.');
        return redirect()->back();
    }   
...
public function edit($id)
{
    ...
    if (!in_array("quotation.update", $decodedData)) {
        Toastr::warning('غير مسموح لك! كلم المدير.');
        return redirect()->back();
    }   
    $quotation = Quotation::with('details')->findOrFail($id);
    ...
    public function destroy($id)
{
    ...
    if (!in_array("quotation.destroy", $decodedData)) {
        Toastr::warning('غير مسموح لك! كلم المدير.');
        return redirect()->back();
    }   
    DB::beginTransaction();
