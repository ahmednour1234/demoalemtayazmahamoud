...
public function index()
{
 ...
 public function getCustomerData($id)
     ...
     */
public function store(Request $request, CustomFieldService $svc): RedirectResponse
{
    // قواعد التحقق الأساسية + قواعد الحقول الإضافية (حسب Customer::class)
    $baseRules = [
        'name'                      => 'required|string|max:255|unique:customers,name',
        'name_en'                   => 'nullable|string|max:255|unique:customers,name_en',
        'mobile'                    => 'required|string|max:50|unique:customers,mobile',
        ...
        'guarantor_images'          => 'nullable|array',
        'guarantor_images.*'        => 'nullable|image|max:2048',
    ];

    $rules = array_merge($baseRules, $svc->rulesFor(\App\Models\Customer::class));
    $data  = $request->validate($rules);

    // رفع صورة العميل (إن وُجدت)، مع fallback للصورة الافتراضية
    $imageName = 'def.png';
    if ($request->hasFile('image')) {
        // لو عندك الهيلبر بتاعك
        $imageName = \Helpers::upload('customer/', 'png', $request->file('image')) ?? 'def.png';
        // أو استخدم التخزين القياسي:
        // $imageName = $request->file('image')->store('uploads/customer', 'public');
    }

    DB::transaction(function () use ($request, $data, $imageName, $svc) {
        $guarantorId = null;

        // إنشاء الضامن لو الاسم موجود
        if (!empty($data['guarantor_name'])) {
            $imagePaths = [];
            if (!empty($data['guarantor_images'])) {
            ...
            $guarantor->name           = $data['guarantor_name'] ?? null;
            $guarantor->national_id    = $data['guarantor_national_id'] ?? null;
            $guarantor->phone          = $data['guarantor_phone'] ?? null;
            $guarantor->address        = $data['guarantor_address'] ?? null;
            $guarantor->job            = $data['guarantor_job'] ?? null;
            $guarantor->monthly_income = $data['guarantor_monthly_income'] ?? null;
            $guarantor->relation       = $data['guarantor_relation'] ?? null;
            $guarantor->images         = json_encode($imagePaths, JSON_UNESCAPED_UNICODE);
            $guarantor->save();
        ...
        $customer->name_en      = $data['name_en'] ?? null;
        $customer->region_id    = $data['region_id'] ?? 1;
        $customer->mobile       = $data['mobile'];
        $customer->email        = $data['email'] ?? null;
        $customer->image        = $imageName;

        $customer->state        = $request->input('state', 1);
        $customer->city         = $request->input('city', 1);
        $customer->zip_code     = $request->input('zip_code', '');
        $customer->address      = $request->input('address', '');

        $customer->longitude    = $data['longitude'] ?? '';
        $customer->latitude     = $data['latitude'] ?? '';
        $customer->type         = $data['type'] ?? 1;
        $customer->category_id  = $data['category_id'] ?? 1;

        $customer->tax_number   = $request->input('tax_number')??1;
        $customer->c_history    = $request->input('c_history')?? 1;
        $customer->guarantor_id = $guarantorId;
        $customer->save();
        ...
        // حفظ الحقول الإضافية داخل نفس الترانزاكشن
        $svc->save($customer, $request->input('custom_fields', []));

        // معاملة مالية أولية (لو عندك لوجيك معين)
...
$accountcustomer=Account::where('id',$customer->account_id)->first();
     ...
     * @return RedirectResponse
     */
    public function update(Request $request,CustomFieldService $svc): RedirectResponse
    {
        // جلب العميل
