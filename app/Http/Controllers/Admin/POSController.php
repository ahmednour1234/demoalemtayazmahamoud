...
public function order_list(Request $request)
{
    ...
    return view('admin-views.pos.order.list', compact(
        ...
        'regions','regionId','orderAmountSum','collectedCashSum',
        'quantitySum','productCount','done','type',
        'sellers','customers','branches',
        'sellerw','customerw','branchw',
    ...
    try {
        ...
        foreach ($orderProducts as $product) {
            ...
            if ($newReturn > 0) {
                ...
                $chosenUnit = $returnUnits[$pid] ?? 1; // 1 كبير، 0 صغير
                ...
                if (($alreadyReturned + $newReturnConverted) > $availableQty) {
                    ...
                    \Toastr::error(
                        translate("لقد قمت بإرجاع {$alreadyReturned} من المنتج {$product->product->name}. المتاح للإرجاع هو {$availableToReturn}")
                    );
                    return redirect()->back();
                }
            }
        ...
        $totalPriceAllProducts = 0; // تكلفة شراء مرتجع (للمخزون)
        ...
        foreach ($orderProducts as $product) {
            ...
            if ($returnQuantity > 0) {
                ...
                $productLog->type       = $returnType;
                $productLog->seller_id  = auth('admin')->user()->id;
                $productLog->branch_id  = auth('admin')->user()->branch_id;
                $productLog->save();

                $stockBatch = new \App\Models\StockBatch;
                $stockBatch->product_id = $pid;
                $stockBatch->quantity   = $finalQuantity;
                $stockBatch->branch_id  = auth('admin')->user()->branch_id;
                $stockBatch->price      = $product->product->purchase_price;
                $stockBatch->save();
            }
        }

        // ========= 4) إنشاء أمر المرتجع + ملاحظة/مرفق =========
        ...
        if ($installment_contract) {
            ...
            $returnRatio      = $totalReturnOverall / max(1, $oldOrder->order_amount);
            ...
            $installment_contract->status = 'cancelled';
        ...
        // ========= 6) إنشاء قيد يومية واحد (Journal Entry) =========
        $customer         = \App\Models\Customer::find($oldOrder->user_id);
        $accCustomer      = \App\Models\Account::find($customer->account_id);
        $accSalesReturn   = \App\Models\Account::find(40); // مردودات المبيعات
        $accTax           = \App\Models\Account::find(28); // ضريبة
        $branch           = \App\Models\Branch::find(auth('admin')->user()->branch_id);
        $accCOGS          = \App\Models\Account::find(47); // تكلفة المبيعات
        $accStock         = \App\Models\Account::find($branch->account_stock_Id); // مخزون
        $accFinanceIncome = \App\Models\Account::find(100); // إيراد تمويلي
        ...
        // مردودات (مدين) + ضريبة (مدين) + عميل (دائن)
        if ($netWithoutTax > 0) {
            $add($accSalesReturn->id, $netWithoutTax, 0,  'مردودات مبيعات (صافي)');
        }
        if ($totalReturnTax > 0) {
            $add($accTax->id, $totalReturnTax, 0,        'ضريبة على المرتجع');
        }
        if ($totalReturnOverall > 0) {
            $add($accCustomer->id, 0, $totalReturnOverall, 'ذمم عملاء مقابل المرتجع (صافي + ضريبة)');
        }

        // مخزون/تكلفة (إن وُجدت تكلفة)
        if ($totalPriceAllProducts > 0) {
        ...
        // إيراد تمويلي عكسي (تقسيط)
        if ($returnedFinanceIncome > 0 && $accFinanceIncome) {
            $add($accFinanceIncome->id, $returnedFinanceIncome, 0, 'عكس إيراد تمويلي ناتج عن المرتجع');
            $add($accCustomer->id, 0, $returnedFinanceIncome,      'ذمم عملاء مقابل عكس الإيراد التمويلي');
        }

        if (!empty($lines)) {
            \DB::table('journal_entries_details')->insert($lines);

            // تحديث إجمالي مدين/دائن في الرأس
            $totD = array_sum(array_column($lines, 'debit'));
            $totC = array_sum(array_column($lines, 'credit'));
            \DB::table('journal_entries')->where('id', $journalId)
                ->update(['updated_at' => now()]);
        }

        // ========= 7) ترانزاكشنز لكل حساب (كما طلبت) =========
        $makeTxn = function($mainAcc, $pairAcc, $amount, $isDebitOnMain, $desc, $type, $customerId, $orderId, $costId = null) {
            ...
            $t->tran_type      = $type;
            $t->seller_id      = auth('admin')->user()->id;
            ...
            // احتفظنا بمنطق الأرصدة عندك
            $t->balance        = $mainAcc->balance - $amount;
            $t->debit_account  = 0;
            $t->credit_account = $amount;
            $t->balance_account= $pairAcc ? ($pairAcc->balance - $amount) : 0;
            $t->date           = date("Y/m/d");
        ...
        // صافي بدون ضريبة
        if ($netWithoutTax > 0) {
            $desc1 = "فاتورة مرتجع بيع (صافي) [JE:$journalRef]";
            $makeTxn($accCustomer, $accSalesReturn, $netWithoutTax, true,  $desc1, $newOrder->type, $oldOrder->user_id, $newOrder->id, $request->cost_id);
            $desc2 = "فاتورة مرتجع بيع (مردودات) [JE:$journalRef]";
            $makeTxn($accSalesReturn, $accCustomer, $netWithoutTax, false, $desc2, $newOrder->type, $oldOrder->user_id, $newOrder->id, $request->cost_id);

            $accCustomer->balance      -= $netWithoutTax; $accCustomer->total_out += $netWithoutTax; $accCustomer->save();
            $accSalesReturn->balance   -= $netWithoutTax; $accSalesReturn->total_out += $netWithoutTax; $accSalesReturn->save();
        }

        // الضريبة
        if ($totalReturnTax > 0) {
            $desc3 = "فاتورة مرتجع بيع (ضريبة) [JE:$journalRef]";
            $makeTxn($accCustomer, $accTax, $totalReturnTax, true,  $desc3, $newOrder->type, $oldOrder->user_id, $newOrder->id);
            $desc4 = "فاتورة مرتجع بيع (طرف ضريبة) [JE:$journalRef]";
            $makeTxn($accTax, $accCustomer, $totalReturnTax, false, $desc4, $newOrder->type, $oldOrder->user_id, $newOrder->id);

            $accCustomer->balance -= $totalReturnTax; $accCustomer->total_out -= $totalReturnTax; $accCustomer->save();
            $accTax->balance      -= $totalReturnTax; $accTax->total_out      -= $totalReturnTax; $accTax->save();
        }

        // المخزون/التكلفة
        if ($totalPriceAllProducts > 0) {
            $desc5 = "فاتورة مرتجع (زيادة مخزون) [JE:$journalRef]";
            $makeTxn($accCOGS, $accStock, $totalPriceAllProducts, true,  $desc5, $newOrder->type, $oldOrder->user_id, $newOrder->id, $request->cost_id);
            $desc6 = "فاتورة مرتجع (عكس تكلفة) [JE:$journalRef]";
            $makeTxn($accStock, $accCOGS, $totalPriceAllProducts, false, $desc6, $newOrder->type, $oldOrder->user_id, $newOrder->id, $request->cost_id);
        ...
        // إيراد تمويلي عكسي
        if ($returnedFinanceIncome > 0 && $accFinanceIncome) {
            $desc7 = "عكس إيراد تمويلي (مرتجع أجل) [JE:$journalRef]";
            $makeTxn($accCustomer, $accFinanceIncome, $returnedFinanceIncome, true,  $desc7, $newOrder->type, $oldOrder->user_id, $oldOrder->id, $request->cost_id);
            $desc8 = "عكس إيراد تمويلي - طرف 100 [JE:$journalRef]";
            $makeTxn($accFinanceIncome, $accCustomer, $returnedFinanceIncome, false, $desc8, $newOrder->type, $oldOrder->user_id, $oldOrder->id, $request->cost_id);

            $accCustomer->balance      -= $returnedFinanceIncome; $accCustomer->total_out    += $returnedFinanceIncome; $accCustomer->save();
            $accFinanceIncome->balance -= $returnedFinanceIncome; $accFinanceIncome->total_out += $returnedFinanceIncome; $accFinanceIncome->save();

            // تحديث رصيد العميل
            $customer->balance += $returnedFinanceIncome; $customer->save();
        }

        // تحديث رصيد العميل بإجمالي المرتجع
        $customer->balance += $totalReturnOverall; $customer->save();

        // ========= 8) تفاصيل الطلب الجديد =========
        ...
        // تحديث الكميات المرتجعة في الفاتورة القديمة (بوحدة كبيرة)
        foreach ($productsReturnData as $p) {
            $oldDetail = $oldOrder->details->where('product_id', $p['product_id'])->first();
            if ($oldDetail) {
                $details   = json_decode($oldDetail->product_details);
                $unitValue = $details->unit_value ?? 1;
                $returnQtyInBase = ($p['unit'] == 0) ? ($p['return_quantity'] / $unitValue) : $p['return_quantity'];
...
public function processConfirmedReturn_service(Request $request)
{
    ...
    try {
        ...
        // إجمالي خصومات الأصناف الأصلية
        $totalProductDiscount = 0;
        foreach ($orderProducts as $product) {
            $totalProductDiscount += $product->discount_on_product * $product->quantity;
        }

        // لحساب نسبة الخصم الإضافي
        $baseOrderAmount = session('order_amount') ?? 0;
        $orderAmount     = $baseOrderAmount + $extraDiscount + $totalProductDiscount - $totalTax;
        $orderAmount     = $orderAmount > 0 ? $orderAmount : 1;
        $discountRatio   = ($extraDiscount / $orderAmount) * 100;

        // كميات ووحدات المرتجع (بالخدمات مفيش تحويل وحدات)
        $returnQuantities = $request->input('return_quantities_hidden', []); // [product_id => qty]
        $returnUnits      = $request->input('return_unit_hidden', []);       // مهمّة للتوحيد فقط
        $returnType       = $request->input('type', 7);
        ...
        // ===== 2) تحقق الكميات (بدون وحدات صغرى/كبرى) =====
        foreach ($orderProducts as $product) {
            $pid       = $product->product_id;
            $newReturn = isset($returnQuantities[$pid]) ? (float)$returnQuantities[$pid] : 0;
            ...
            if ($newReturn > 0) {
                $oldDetail = $oldOrder->details->where('product_id', $pid)->first();
                $alreadyReturned = $oldDetail ? ($oldDetail->quantity_returned ?? 0) : 0;
        ...
        $totalReturnTax = 0;
        $totalReturnOverall = 0;
        $returnedFinanceIncome = 0.0; // لو كان في تقسيط

        foreach ($orderProducts as $product) {
            $pid         = $product->product_id;
            $chosenUnit  = $returnUnits[$pid] ?? 1; // للخدمات: 1 دايمًا عمليًا

            // لا تحويل وحدات في الخدمات
            $adjustedPrice         = $product->price;
            $adjustedDiscount      = $product->discount_on_product;
            $adjustedExtraDiscount = ($discountRatio / 100) * $product->price;
            $adjustedTax           = $product->tax_amount;
        ...
        // ===== 4) إنشاء أمر المرتجع (Service) + ملاحظة/مرفق =====
        $newOrder = new \App\Models\Order;
        $newOrder->owner_id   = auth('admin')->user()->id;
        $newOrder->user_id    = $oldOrder->user_id;
        $newOrder->parent_id  = $oldOrder->id;
        ...
        // QR
        $qrcodeData  = "https://demo.novoosystem.com/real/invoicea2/" . $newOrder->id;
        $qrCode      = new \Endroid\QrCode\QrCode($qrcodeData);
        $writer      = new \Endroid\QrCode\Writer\PngWriter();
        $qrcodeImage = $writer->write($qrCode)->getString();
        $qrcodePath  = "qrcodes/order_" . $newOrder->id . ".png";
        ...
        if ($installment_contract) {
            ...
            // قيود عكس الإيراد التمويلي (سيُعالج في اليومية والترانزاكشنز تحت)
        }

        // ===== 6) إنشاء قيد يومية (واحد) بالرأس والتفاصيل =====
        $customer         = \App\Models\Customer::find($oldOrder->user_id);
        $accCustomer      = \App\Models\Account::find($customer->account_id);
        $accSalesReturn   = \App\Models\Account::find(40);  // مردودات المبيعات (للخدمات نستخدمه كـ عكس إيراد)
        $accTax           = \App\Models\Account::find(28);  // ضريبة
        $accFinanceIncome = \App\Models\Account::find(100); // إيراد تمويلي

        $journalRef = 'RET-SRV-' . now()->format('YmdHis') . '-' . $newOrder->id;
        $headDate   = $request->filled('date') ? \Carbon\Carbon::parse($request->date)->toDateString() : now()->toDateString();
        $admin      = auth('admin')->user();
        $adminId    = $admin->id;

        // رأس القيد (مطابق للـ schema اللي بعته)
        $journalId = \DB::table('journal_entries')->insertGetId([
            'entry_date' => $headDate,
            'reference'  => $journalRef,
            'description'=> 'قيد يومية مرتجع خدمة لفاتورة #' . $newOrder->id,
            'created_by' => $adminId,
        ...
        $attachmentFile = $request->file('attachment'); // هنرفقه في أول سطر مثلاً
        $lines = [];

        $addLine = function($accountId, $debit, $credit, $memo, $withAttachment = false) use (&$lines, $journalId, $request, $attachmentFile) {
            if (round($debit,2)==0 && round($credit,2)==0) return;

            $img = null;
            if ($withAttachment && $attachmentFile) {
                // نفس أسلوب التخزين اللي طلبته
                $img = \App\CPU\Helpers::update('journal/', null, 'png', $attachmentFile);
            }

            $lines[] = [
                'journal_entry_id' => $journalId,
                'account_id'       => $accountId,
                'cost_center_id'   => $request->input('cost_id'),
                'debit'            => round($debit, 2),
                'credit'           => round($credit, 2),
                'description'      => $memo,
                'attachment_path'  => $img,
        ...
        // مردودات/ضريبة/عميل
        if ($netWithoutTax > 0) {
            $addLine($accSalesReturn->id, $netWithoutTax, 0, 'مردودات خدمات (صافي)', true);
        }
        if ($totalReturnTax > 0) {
            $addLine($accTax->id, $totalReturnTax, 0, 'ضريبة على مرتجع خدمات', false);
        }
        if ($totalReturnOverall > 0) {
            $addLine($accCustomer->id, 0, $totalReturnOverall, 'ذمم عملاء مقابل مرتجع خدمات', false);
        }

        // عكس إيراد تمويلي إن وجد
        if ($returnedFinanceIncome > 0 && $accFinanceIncome) {
            $addLine($accFinanceIncome->id, $returnedFinanceIncome, 0, 'عكس إيراد تمويلي للمرتجع', false);
            $addLine($accCustomer->id, 0, $returnedFinanceIncome, 'ذمم عملاء مقابل عكس الإيراد التمويلي', false);
        }
        ...
        $makeTxn = function($mainAcc, $pairAcc, $amount, $isDebitOnMain, $desc, $type, $customerId, $orderId, $costId = null) {
            ...
            $t->debit          = $isDebitOnMain ? $amount : 0;
            $t->credit         = $isDebitOnMain ? 0 : $amount;
            // نفس منطق الرصيد كما في كودك الأصلي
            $t->balance        = $mainAcc->balance - $amount;
            $t->debit_account  = 0;
            $t->credit_account = $amount;
            $t->balance_account= $pairAcc ? ($pairAcc->balance - $amount) : 0;
        ...
        // صافي بدون ضريبة
        if ($netWithoutTax > 0) {
            $desc1 = "مرتجع خدمة (صافي) [JE:$journalRef]";
            $makeTxn($accCustomer,    $accSalesReturn, $netWithoutTax, true,  $desc1, $newOrder->type, $oldOrder->user_id, $newOrder->id, $request->cost_id);
            $desc2 = "مرتجع خدمة (مردودات) [JE:$journalRef]";
            $makeTxn($accSalesReturn, $accCustomer,    $netWithoutTax, false, $desc2, $newOrder->type, $oldOrder->user_id, $newOrder->id, $request->cost_id);
        ...
        // الضريبة
        if ($totalReturnTax > 0) {
            $desc3 = "مرتجع خدمة (ضريبة) [JE:$journalRef]";
            $makeTxn($accCustomer, $accTax, $totalReturnTax, true,  $desc3, $newOrder->type, $oldOrder->user_id, $newOrder->id);
            $desc4 = "مرتجع خدمة (طرف ضريبة) [JE:$journalRef]";
            $makeTxn($accTax,      $accCustomer, $totalReturnTax, false, $desc4, $newOrder->type, $oldOrder->user_id, $newOrder->id);
        ...
        // عكس الإيراد التمويلي (لو تقسيط)
        if ($returnedFinanceIncome > 0 && $accFinanceIncome) {
            $desc5 = "عكس إيراد تمويلي (مرتجع خدمة) [JE:$journalRef]";
            $makeTxn($accCustomer,      $accFinanceIncome, $returnedFinanceIncome, true,  $desc5, $newOrder->type, $oldOrder->user_id, $oldOrder->id, $request->cost_id);
            $desc6 = "عكس إيراد تمويلي - طرف 100 [JE:$journalRef]";
            $makeTxn($accFinanceIncome, $accCustomer,      $returnedFinanceIncome, false, $desc6, $newOrder->type, $oldOrder->user_id, $oldOrder->id, $request->cost_id);

            $accCustomer->balance      -= $returnedFinanceIncome;  $accCustomer->total_out    += $returnedFinanceIncome;  $accCustomer->save();
            $accFinanceIncome->balance -= $returnedFinanceIncome;  $accFinanceIncome->total_out += $returnedFinanceIncome; $accFinanceIncome->save();

            // تحديث رصيد العميل (كما في كودك)
            $customer->balance += $returnedFinanceIncome; $customer->save();
        }
        ...
        // تحديث الكميات المرتجعة في الفاتورة الأصلية (بدون تحويل وحدات)
        foreach ($productsReturnData as $p) {
            $oldDetail = $oldOrder->details->where('product_id', $p['product_id'])->first();
            if ($oldDetail) {
                $oldDetail->quantity_returned = ($oldDetail->quantity_returned ?? 0) + $p['return_quantity'];
                $oldDetail->save();
            }
        }
