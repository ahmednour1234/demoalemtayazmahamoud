   ...
   public function add(Request $request, $type)
{
    ...
    if (!in_array("Expense.store", $decodedData)) {
        Toastr::warning('غير مسموح لك! كلم المدير.');
        return redirect()->back();
    }       
    ...
    if (!in_array("expense100.store", $decodedData)) {
        Toastr::warning('غير مسموح لك! كلم المدير.');
        return redirect()->back();
    }      
    }elseif($type==200){
    ...
    if (!in_array("expense200.store", $decodedData)) {
        Toastr::warning('غير مسموح لك! كلم المدير.');
        return redirect()->back();
    }    
    }
...
public function getVouchers(Request $request,$type)
{
    ...
    if (!in_array("finance.reports.payment_vouchers", $decodedData)) {
        Toastr::warning('غير مسموح لك! كلم المدير.');
        return redirect()->back();
    }  
    ...
    if (!in_array("finance.reports.receipt_vouchers", $decodedData)) {
        Toastr::warning('غير مسموح لك! كلم المدير.');
        return redirect()->back();
    }  
