    ...
    public function index(Request $request)
{
    ...
    if (!in_array("finance.reports.journal_entries", $decodedData)) {
        Toastr::warning('غير مسموح لك! كلم المدير.');
        return redirect()->back();
    }  
    ...
    if (!in_array("finance.reports.journal_entries.show", $decodedData)) {
        Toastr::warning('غير مسموح لك! كلم المدير.');
        return redirect()->back();
    }  
    ...
    if (!in_array("finance.reports.journal_entries.update", $decodedData)) {
        Toastr::warning('غير مسموح لك! كلم المدير.');
        return redirect()->back();
    }  
    ...
    if (!in_array("finance.reports.journal_entries.reserve", $decodedData)) {
        Toastr::warning('غير مسموح لك! كلم المدير.');
        return redirect()->back();
    }  
...
public function softDeleteJournalEntry(Request $request, int $id)
{
    // ===== صلاحيات (غير إلزامي: عدّل المفتاح حسب نظامك) =====
    $adminId = Auth::guard('admin')->id();
    $admin = DB::table('admins')->where('id', $adminId)->first();
    ...
    if (!is_array($decodedData) || !in_array("finance.reports.journal_entries.soft_delete", $decodedData)) {
        Toastr::warning('غير مسموح لك! كلم المدير.');
        return back();
    }

    $request->validate(['hard_recalc' => 'nullable|boolean']);
    ...
    try {
        ...
        $entry = JournalEntry::with([
                'details' => fn($q) => $q->whereNull('deleted_at'),
            ])->lockForUpdate()->findOrFail($id);

        if ($entry->trashed()) {
            DB::rollBack();
            Toastr::info('القيد محذوف لطيفًا بالفعل.');
            return back();
        }

        // لو القيد الأصلي تم عكسه مسبقًا نسمح بالحذف اللطيف لكن سنطرَح أثر القيد الأصلي فقط.
        // (إن أردت المنع أضف شرطًا هنا)

        // 1) اطرح أثر القيد من الأرصدة (sign = -1)
        [$accIds, $suppIds, $custIds] = $this->applyEntryEffectSign($entry, -1);

        // 2) SoftDelete للمعاملات ثم التفاصيل ثم رأس القيد
        foreach ($entry->details as $detail) {
            $txs = Transection::where('journal_entry_detail_id', $detail->id)
        ...
        // 3) (اختياري) إعادة احتساب شاملة
        if ($request->boolean('hard_recalc')) {
            $this->hardRecalcAccounts($accIds);
            $this->hardRecalcSuppliers($suppIds);
            $this->hardRecalcCustomers($custIds);
        }

        DB::commit();
        Toastr::success('تم أرشفة (حذف لطيف) القيد وطرح أثره من الأرصدة.');
        return back();
    } catch (\Throwable $e) {
        DB::rollBack();
        dd($e);
        \Log::error('JournalEntry soft delete failed', ['entry_id' => $id, 'error' => $e->getMessage()]);
        Toastr::error('فشل الحذف اللطيف! حاول لاحقًا.');
        return back();
    }
}
protected function applyEntryEffectSign(JournalEntry $entry, int $sign = +1): array
{
    $touchedAccountIds  = [];
    $touchedSupplierIds = [];
    $touchedCustomerIds = [];
    ...
    foreach ($entry->details as $detail) {
        // المعاملات النشطة فقط
        $txs = Transection::where('journal_entry_detail_id', $detail->id)
            ->whereNull('deleted_at')->get();

        foreach ($txs as $tx) {
            // لو sign = +1: نفس اتجاه المعاملة
            // لو sign = -1: نعكس تأثيرها (نطرح الأثر القديم)
            // في الحالتين نستخدم bumpAccountTotals بالاتجاه الأصلي لكن بقيمة موجبة أو سالبة حسب sign
            $this->bumpAccountTotals($tx->account_id,    $sign * $tx->amount, 'in');  // مدين
            $this->bumpAccountTotals($tx->account_id_to, $sign * $tx->amount, 'out'); // دائن

            $touchedAccountIds[$tx->account_id]    = true;
            $touchedAccountIds[$tx->account_id_to] = true;

            // تعديل أطراف (اختياري بحسب منطقك)
            $this->bumpPartyByAccountsForSoft($tx, $sign, $touchedSupplierIds, $touchedCustomerIds);
        }
    }

    return [array_keys($touchedAccountIds), array_keys($touchedSupplierIds), array_keys($touchedCustomerIds)];
}

/**
 * (اختياري) تعديل أثر عميل/مورد عند soft delete/restore
 * استخدم sign = +1 للإضافة، -1 للطرح.
 */
protected function bumpPartyByAccountsForSoft(Transection $tx, int $sign, array &$suppIds, array &$custIds): void
{
    // إن كان عندك منطق يربط حسابات محددة بـ Customers/Suppliers عدّله هنا.
    // مثال (افتراضي):
    // if ($this->isSupplierAccount($tx->account_id)) {
    //     $this->bumpSupplierTotals($this->supplierIdFromAccount($tx->account_id), $sign * $tx->amount);
    //     $suppIds[$this->supplierIdFromAccount($tx->account_id)] = true;
    // }
    // if ($this->isCustomerAccount($tx->account_id)) {
    //     $this->bumpCustomerTotals($this->customerIdFromAccount($tx->account_id), $sign * $tx->amount);
    //     $custIds[$this->customerIdFromAccount($tx->account_id)] = true;
    // }
}

/**
 * مجرد غطاء: زيادة/نقصان الإجماليات والرصيد للحساب.
 * $direction: 'in' للمدين، 'out' للدائن.
 * نفذها بما يتماشى مع تركيب جدول accounts لديك.
 */
// protected function bumpAccountTotals(int $accountId, float $amount, string $direction): void
// {
//     // مثال سريع: لو عندك أعمدة total_in, total_out, balance
//     if ($direction === 'in') {
//         DB::table('accounts')->where('id', $accountId)->update([
//             'total_in' => DB::raw('COALESCE(total_in,0) + '.($amount)),
//             'balance'  => DB::raw('COALESCE(balance,0) + '.($amount)),
//             'updated_at' => now(),
//         ]);
//     } else {
//         DB::table('accounts')->where('id', $accountId)->update([
//             'total_out' => DB::raw('COALESCE(total_out,0) + '.($amount)),
//             'balance'   => DB::raw('COALESCE(balance,0) - '.($amount)),
//             'updated_at' => now(),
//         ]);
//     }
...
/**
 * دوال إعادة احتساب شاملة (موجودة عندك مسبقًا).
 * لو مش عندك، اتركني أكتبها لك لاحقًا حسب بنية مشروعك.
 */
