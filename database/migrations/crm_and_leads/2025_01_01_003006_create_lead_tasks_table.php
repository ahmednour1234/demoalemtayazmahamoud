...
class CreateLeadTasksTable extends Migration
{
    ...
    public function up()
    {
        ...
        DB::statement("CREATE TABLE `lead_tasks` ( `id` bigint(20) UNSIGNED NOT NULL, `lead_id` bigint(20) UNSIGNED NOT NULL, `title` varchar(190) NOT NULL, `due_at` datetime DEFAULT NULL, `assigned_to_id` bigint(20) UNSIGNED DEFAULT NULL, `status` enum('pending','done','canceled') NOT NULL DEFAULT 'pending', `priority` enum('low','normal','high','urgent') NOT NULL DEFAULT 'normal', `created_by_id` bigint(20) UNSIGNED DEFAULT NULL, `created_at` timestamp NULL DEFAULT NULL, `updated_at` timestamp NULL DEFAULT NULL, `deleted_at` timestamp NULL DEFAULT NULL ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;");
        DB::statement("ALTER TABLE `lead_tasks` ADD PRIMARY KEY (`id`), ADD KEY `idx_tasks_lead` (`lead_id`), ADD KEY `idx_tasks_due` (`due_at`), ADD KEY `idx_tasks_assg` (`assigned_to_id`), ADD KEY `fk_tasks_crby` (`created_by_id`);");
        DB::statement("ALTER TABLE `lead_tasks` MODIFY `id` bigint(20) UNSIGNED NOT NULL AUTO_INCREMENT;");
        DB::statement("ALTER TABLE `lead_tasks` ADD CONSTRAINT `fk_tasks_assg` FOREIGN KEY (`assigned_to_id`) REFERENCES `admins` (`id`) ON DELETE SET NULL, ADD CONSTRAINT `fk_tasks_crby` FOREIGN KEY (`created_by_id`) REFERENCES `admins` (`id`) ON DELETE SET NULL, ADD CONSTRAINT `fk_tasks_lead` FOREIGN KEY (`lead_id`) REFERENCES `leads` (`id`) ON DELETE CASCADE;");
