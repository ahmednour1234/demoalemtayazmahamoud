...
use App\Http\Controllers\Admin\CallOutcomeController;
...
Route::get('/admin/customer/details/{id}', [CustomerController::class, 'getDetails'])->name('admin.customer.details');
    Route::get('/accounts/totals', [AccountController::class, 'totals'])
        ->name('admin.accounts.totals');
...
Route::get('/', function () {
    ...
    // التعليقات (إضافة/حذف) — استخدمنا نفس الكنترولر بميثودين صغيرين بالأسفل
    Route::post('tickets/{ticket}/comments',  [TicketController::class, 'commentStore'])->name('tickets.comments.store');
    Route::delete('tickets/{ticket}/comments/{comment}', [TicketController::class, 'commentDestroy'])->name('tickets.comments.destroy');
///////////////////projects///////////////
  Route::get('projects',            [\App\Http\Controllers\Admin\ProjectController::class, 'index'])->name('projects.index');
    ...
    Route::patch('projects/{project}/active', [\App\Http\Controllers\Admin\ProjectController::class, 'active'])->name('projects.active');
    Route::prefix('projects/{project}')->group(function () {
        ...
        Route::put('members/{member}',       [ProjectMemberController::class, 'update'])->name('projects.members.update');
    ...
    Route::patch('tasks/{task}/active', [\App\Http\Controllers\Admin\TaskController::class, 'active'])->name('tasks.active');
    ...
    Route::patch('statuses/{status}/active', [StatusController::class, 'active'])->name('status.active');
    
    
     // Follow Ups
    Route::get('followups',                 [FollowUpController::class, 'index'])->name('followups.index');
    Route::get('followups/create',          [FollowUpController::class, 'create'])->name('tasks.followups.create');
    Route::post('followups',                [FollowUpController::class, 'store'])->name('tasks.followups.store');
    Route::get('followups/{followup}/edit', [FollowUpController::class, 'edit'])->name('tasks.followups.edit');
    Route::put('followups/{followup}',      [FollowUpController::class, 'update'])->name('tasks.followups.update');
    Route::delete('followups/{followup}',   [FollowUpController::class, 'destroy'])->name('tasks.followups.destroy');

    // Comments (بسيط: حفظ وحذف)
    ...
    Route::delete('approvals/{approval}',   [ApprovalController::class, 'destroy'])->name('approvals.destroy');
        Route::post('/tasks/{task}/comments', [\App\Http\Controllers\Admin\TaskCommentController::class,'store'])
        ->name('tasks.comments.store'); // permission: task.comments.store
    Route::delete('/tasks/{task}/comments/{comment}', [\App\Http\Controllers\Admin\TaskCommentController::class,'destroy'])
        ->name('tasks.comments.destroy');

    // المتابعات (Follow-ups) الخاصة بالمهمة
    Route::post('/tasks/{task}/followups', [\App\Http\Controllers\Admin\TaskFollowUpController::class,'store'])
        ->name('tasks.followups.store'); // permission: task.followups.store
    Route::post('/tasks/{task}/followups/{followup}/done', [\App\Http\Controllers\Admin\TaskFollowUpController::class,'done'])
        ->name('tasks.followups.done');
    Route::delete('/tasks/{task}/followups/{followup}', [\App\Http\Controllers\Admin\TaskFollowUpController::class,'destroy'])
        ->name('tasks.followups.destroy');
/////////////////////////////
...
Route::get('/vouchers/show/{voucher_number}', [ExpenseController::class, 'showVouchers'])
    ...
    Route::delete('{id}', [\App\Http\Controllers\Admin\JournalEntryController::class, 'softDeleteJournalEntry'])
                    ->whereNumber('id')
                    ->name('journal-entries.soft_delete');
...
/////////////////////////////////////////////////Leads///////////////////////////////////////////////////////////////////////////////
Route::prefix('lead-statuses')
    ->name('lead-statuses.')
    ->group(function () {
        Route::get('/',            [LeadStatusController::class, 'index'])->name('index');
        Route::get('/create',      [LeadStatusController::class, 'create'])->name('create');
        Route::post('/',           [LeadStatusController::class, 'store'])->name('store');
        Route::get('/{status}/edit', [LeadStatusController::class, 'edit'])->name('edit');
        Route::put('/{status}',    [LeadStatusController::class, 'update'])->name('update');
        Route::delete('/{status}', [LeadStatusController::class, 'destroy'])->name('destroy');

        // تفعيل/تعطيل
        Route::patch('/{status}/active', [LeadStatusController::class, 'active'])->name('active');
    });
    

Route::prefix('lead-sources')
    ->name('lead-sources.')
    ->group(function () {
        ...
        Route::patch('/{source}/active', [LeadSourceController::class, 'active'])->name('active');
    });    
...
Route::prefix('leads')
    ...
    ->group(function () {
        ...
        Route::get('/template',       [LeadController::class, 'template'])->name('template');

        // Import
        Route::get('/import',         [LeadController::class, 'importView'])->name('import.view');
        Route::post('/import',        [LeadController::class, 'import'])->name('import');

        // Auto-assign unassigned evenly
        Route::post('/auto-assign',   [LeadController::class, 'autoAssign'])->name('auto_assign');

        // Show page with tabs
        Route::get('/{lead}',         [LeadController::class, 'show'])->name('show');
    });
    Route::prefix('call-outcomes')->name('call-outcomes.')->group(function () {
    Route::get('/',                [CallOutcomeController::class, 'index'])->name('index');
    Route::get('/create',          [CallOutcomeController::class, 'create'])->name('create');
    Route::post('/',               [CallOutcomeController::class, 'store'])->name('store');
    Route::get('/{outcome}/edit',  [CallOutcomeController::class, 'edit'])->name('edit');
    Route::put('/{outcome}',       [CallOutcomeController::class, 'update'])->name('update');
    Route::delete('/{outcome}',    [CallOutcomeController::class, 'destroy'])->name('destroy');
    Route::patch('/{outcome}/active', [CallOutcomeController::class, 'active'])->name('active');
    Route::get('/export',          [CallOutcomeController::class, 'export'])->name('export');
});
...
Route::prefix('call-logs')->name('call-logs.')->group(function () {
    ...
    Route::get('/{log}/edit',   [CallLogController::class, 'edit'])->name('edit');
    Route::put('/{log}',        [CallLogController::class, 'update'])->name('update');
    Route::delete('/{log}',     [CallLogController::class, 'destroy'])->name('destroy');
    Route::get('/export',       [CallLogController::class, 'export'])->name('export');
});

Route::resource('custom-fields', CustomFieldController::class);
Route::patch('custom-fields/{customField}/active', [CustomFieldController::class, 'active'])->name('custom-fields.active');
    Route::resource('lead-notes', \App\Http\Controllers\Admin\LeadNoteController::class)
        ->parameters(['lead-notes' => 'lead_note']) // لتطابق binding
        ->except(['show']);
            Route::get('leads/{lead}/report.pdf', [\App\Http\Controllers\Admin\LeadController::class,'pdfReport'])
    ...
    Route::delete('/{id}', 'InventoryAdjustmentController@destroy')
        ...
        Route::group(['prefix' => 'admin', 'as' => 'admin.'], function () {
            Route::get('add','AdminController@index')->name('add');
                        Route::get('structure','AdminController@structure')->name('structure');
