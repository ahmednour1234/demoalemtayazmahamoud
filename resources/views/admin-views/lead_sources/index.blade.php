{{-- resources/views/admin/lead_sources/index.blade.php --}}
@extends('layouts.admin.app')

@php
  // حل مشكلة Call to undefined function schema():
  // استخدم الـ Facade الرسمي بدلاً من الدالة schema()
  use Illuminate\Support\Facades\Schema;
@endphp

@section('title','مصادر العملاء المحتملين')
...
<div class="content container-fluid">
    ...
    <nav aria-label="breadcrumb">
      <ol class="breadcrumb bg-white px-3 py-2 rounded shadow-sm">
        ...
        <li class="breadcrumb-item active" aria-current="page">
          {{ \App\CPU\translate('مصادر العملاء المحتملين') }}
        </li>
      </ol>
    </nav>
  </div>
  ...
  <div class="d-flex justify-content-between align-items-center mb-3">
    <h3 class="mb-0">{{ \App\CPU\translate('مصادر العملاء المحتملين') }}</h3>
    <a href="{{ route('admin.lead-sources.create') }}" class="btn btn-primary">
      <i class="tio-add"></i> {{ \App\CPU\translate('إضافة مصدر') }}
    </a>
  </div>
      ...
      <table class="table">
        ...
        @forelse($sources as $s)
          <tr>
            ...
            {{-- Button Switch Toggle (بدون checkbox في الواجهة؛ فورم يرسل active=0/1) --}}
            <td>
              @if (Schema::hasColumn('lead_sources','is_active'))
                <form method="post"
                      action="{{ route('admin.lead-sources.active', $s) }}"
              ...
              @else
                <span class="badge bg-secondary">{{ \App\CPU\translate('غير مدعوم') }}</span>
              @endif
            </td>
            ...
            <td class="text-center">
              <div class="d-inline-flex align-items-start">
                <a href="{{ route('admin.lead-sources.edit', $s) }}"
                ...
                <form method="post"
                      action="{{ route('admin.lead-sources.destroy', $s) }}"
                      onsubmit="return confirm('{{ \App\CPU\translate('حذف المصدر؟') }}');"
                      class="d-inline">
                  @csrf @method('DELETE')
                  <button type="submit"
...
<script>
  // تبديل حالة التفعيل بزر واحد (يعدّل قيمة hidden ثم يرسل الفورم)
