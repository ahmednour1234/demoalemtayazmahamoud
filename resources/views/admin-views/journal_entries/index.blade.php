...
@section('content')
@php
    $hasSearch = request('from_date') || request('to_date') || request('branch_id') || request('account_id') || request('seller_id') || request('reference') || request('description');
...
@if($isReversal)
 ...
 {{-- زر الحذف اللطيف --}}
    <button type="button"
            ...
            data-id="{{ $e->id }}"
            data-ref="{{ $e->reference ?? $e->head_ref }}"
            title="{{ __('حذف لطيف (أرشفة)') }}">
        <i class="tio-delete"></i>
    </button>
@endif

                                    </div>
                                    {{-- فورم الحذف اللطيف المخفي --}}
<form id="soft-delete-form-{{ $e->id }}"
      action="{{ route('admin.journal-entries.soft_delete', $e->id) }}"
...
document.addEventListener('click', function (e) {
  ...
  const msg = `هل أنت متأكد من أرشفة القيد ${ref} (حذف لطيف)؟\nيمكن استرجاعه لاحقًا.`;
  if (confirm(msg)) {
    const form = document.getElementById(`soft-delete-form-${id}`);
    if (form) form.submit();
  }
});
</script>
