...
<div class="content container-fluid">
  ...
  <div class="mb-3">
    <nav aria-label="breadcrumb">
      <ol class="breadcrumb bg-white px-3 py-2 rounded shadow-sm mb-0">
        ...
        <li class="breadcrumb-item active">{{ \App\CPU\translate('فواتير المبيعات') }}</li>
      </ol>
    </nav>
  </div>
  ...
  <div class="filter-card mb-3">
    <div class="filter-head">
      <h4>{{ \App\CPU\translate('بحث وفلاتر المبيعات') }}</h4>
      @if($hasFilters)
    ...
    <div class="filter-body">
      <form action="{{ url()->current() }}" method="GET">
        <div class="filter-grid">
          ...
          {{-- المنطقة --}}
          <div class="fg-4">
            <label>{{ \App\CPU\translate('المنطقة') }}</label>
          ...
          {{-- المندوب --}}
          <div class="fg-4">
            <label>{{ \App\CPU\translate('المندوب') }}</label>
          ...
          {{-- العميل --}}
          <div class="fg-4">
            <label>{{ \App\CPU\translate('العميل') }}</label>
...
<script>
  ...
  function print_invoicea2(order_id) {
    $.get({
      url: '{{url('/')}}/admin/pos/invoicea2/' + order_id,
      dataType: 'json',
      beforeSend: function () { $('#loading').show(); },
      success: function (data) {
        $('#print-invoice').modal('show');
