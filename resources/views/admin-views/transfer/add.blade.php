    ...
    <form action="{{ route('admin.account.store-transfer') }}" method="POST" enctype="multipart/form-data" id="journalForm">
        ...
        <div class="table-container" id="tableScrollContainer">
            <table id="journalTable">
                <thead>
                    @php
                        $headers = ['رقم الصف','رقم الحساب', 'الحساب', 'مدين', 'دائن', 'البيان', 'مركز التكلفة', 'الصورة', 'الإجراء'];
                    @endphp
                    <tr>
                        <th class="col-num">{{ $headers[0] }}</th>
                        <th class="col-code">{{ $headers[1] }}</th>
                        <th class="col-acct">{{ $headers[2] }}</th>
                        <th class="col-debit">{{ $headers[3] }}</th>
                        <th class="col-credit">{{ $headers[4] }}</th>
                        <th class="col-desc">{{ $headers[5] }}</th>
                        <th class="col-cost">{{ $headers[6] }}</th>
                        <th class="col-img">{{ $headers[7] }}</th>
                        <th class="col-actions">{{ $headers[8] }}</th>
                    </tr>
                </thead>
                <tbody></tbody>
                <tfoot>
                    <tr>
                        <td colspan="3" class="text-start" style="font-weight:700">إجمالي المدين / الدائن</td>
...
$(document).ready(function () {
    ...
    function generateAccountOptions(data) {
        return data.map(item => {
            ...
            // نعرض الاسم ومعاه الكود لسهولة الفهم داخل القائمة
            return `<option value="${item.id}" data-code="${code}" data-cost-center="${ccReq}">${label} — ${code}</option>`;
        }).join('');
    }
    ...
    // Select2 matcher: يبحث بالاسم أو بالكود
    ...
    function makeRow(i) {
        ...
        <tr>
            ...
            <td>
                <input type="text" class="form-control account-code num" placeholder="كود الحساب" name="entries[${i}][account_code]">
            </td>
            ...
            <td><input type="number" step="0.01" min="0" name="entries[${i}][debit]" class="form-control debit num" value="0"></td>
            <td><input type="number" step="0.01" min="0" name="entries[${i}][credit]" class="form-control credit num" value="0"></td>
            ...
            <!-- Hidden row date (synced from global) -->
            <input type="hidden" name="entries[${i}][date]" class="row-date-hidden" value="">
        </tr>`;
    }
    ...
    function rowNeedsCC($row){
        ...
        return parseInt($acct.find('option:selected').data('cost-center') || 0) === 1;
    }
    ...
    function isRowEmpty($row){
        ...
        return !acct && debit === 0 && credit === 0 && desc === '' && !cost && files === 0;
    }
    ...
    function initSelect2($scope){
        $scope.find('.account-select').select2({ width:'100%', matcher: accountMatcher });
        $scope.find('.cost-select').select2({ width:'100%' });
    }
    ...
    // تعبئة تاريخ اليوم تلقائيًا
    const today = getTodayForInput();
    $('#globalDate').val(today);
    ...
    // منع إدخال قيم في الطرفين معًا
    $(document).on('input', '.debit', function(){
        const v = parseFloat($(this).val()) || 0;
        if(v > 0){ $(this).closest('tr').find('.credit').val('0'); }
        calcTotals();
    });
    $(document).on('input', '.credit', function(){
        const v = parseFloat($(this).val()) || 0;
        if(v > 0){ $(this).closest('tr').find('.debit').val('0'); }
        calcTotals();
    });

    // == ربط "رقم الحساب" ببحث الـ Select المجاور له
    function openSelectAndType($select, term){
        $select.select2('open');
        // inject term into the global search box of the opened Select2
        const $search = $('.select2-container--open .select2-search__field');
        $search.val(term).trigger('input');
    }

    // على كل إدخال في خانة الكود:
    $(document).on('input', '.account-code', function () {
        const codeInput = ($(this).val() || '').trim();
        const $row = $(this).closest('tr');
        const $select = $row.find('.account-select');
        ...
        // 1) فلترة مباشرة داخل الـ Select2 (بالاسم أو بالكود)
        openSelectAndType($select, codeInput);

        // 2) لو فيه تطابق تام مع كود خيار -> اختاره تلقائيًا
        const $exact = $select.find('option').filter(function(){
            return String($(this).data('code') || '').toLowerCase() === codeInput.toLowerCase();
        }).first();
        if ($exact.length) {
            $select.val($exact.val()).trigger('change');
            // أقفل القايمة بعد الاختيار لو حابب
            $select.select2('close');
        }
        calcTotals();
    });

    // لو اختار من القائمة، حدث خانة الكود و متطلبات مركز التكلفة
    ...
    // فلترة الصفوف كلها (أداة toolbar)
